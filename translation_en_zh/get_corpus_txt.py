...
if __name__ == "__main__":  # 主程序入口，当脚本被直接执行时运行
    files = ['train', 'val', 'test']  # 定义文件列表，包含训练集、开发集和测试集
    ch_path = './corpus_zh.txt'  # 中文语料库文件路径
    en_path = './corpus_en.txt'  # 英文语料库文件路径
    ...
    for file in files:  # 遍历文件列表
        ...
        corpus = json.load(open(json_path + '/' + file + '.json', 'r', encoding="utf-8"))
        # 遍历语料中的每一项，提取英文和中文句子
        for item in corpus:
